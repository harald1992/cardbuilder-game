export function createMap() {
  const canvas = document.querySelector("#mapgenerator-canvas");
  //create canvas var
  ...
  function Cell(c, r, x, y) {
    ...
    this.carveH = function (
      dis,
      x,
      y //carve out the horizontal corridor
    ) {
      if (
        this.r >= x &&
        this.r < x + dis &&
    ...
    this.carveV = function (
      dis,
      x,
      y //carve out the vertical corridor
    ) {
      if (
        this.c >= y &&
        this.c < y + dis &&
  ...
  function createRooms() {
    ...
    for (
      var i = 0;
      i < amount;
      i++ //for the amount of rooms you want
    ) {
      var room = new Room(

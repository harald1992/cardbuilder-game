import { $cardDictionary, CardConfig, getCardByTitles } from "../dictionaries/card-dictionary";
...
export class Deck {
    ...
    update(deltaTime: number) {
        // this.allCards = this.allCards.filter((o) => !o.markedForDeletion);
        // this.cardsInDeck = this.cardsInDeck.filter((o) => !o.markedForDeletion);
        // this.cardsInDiscard = this.cardsInDiscard.filter((o) => !o.markedForDeletion);
        // this.cardsInHand = this.cardsInHand.filter((o) => !o.markedForDeletion);
        ...
        if (this.cardsInHand.length !== this.lastCardAmount) {
            this.cardsInHand.forEach((card: Card, index: number) => {
                ...
                card.y = this.unit.isPlayer ? this.unit.game.main.height - card.height : 0;
            });
    ...
    drawCards(amount = 5) {
        for (let i = 0; i < amount; i++) {
            if (this.cardsInDeck.length === 0) { this.shuffleDiscardPileInDeck(); }
    ...
    shuffleDiscardCardInDeck(cardToShuffleBack: Card) {
        ...
        this.cardsInDiscard = this.cardsInDiscard.filter((card: Card) => card.id !== cardToShuffleBack.id);
    ...
    discardCard(cardToDiscard: Card) {
        ...
        this.cardsInHand = this.cardsInHand.filter((card: Card) => card.id !== cardToDiscard.id);
